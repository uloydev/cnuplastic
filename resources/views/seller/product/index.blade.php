                        ...
                        <tbody>
                            @forelse ($products as $product)
                                <tr>
                                    ...
                                    <td>
                                        <form method="post" id="deleteForm{{ $product->id }}" action="{{ route('seller.product.destroy', $product->id) }}">
                                            @csrf
                                            @method("DELETE")
                                        </form>
                                        <a href="{{ route('seller.product.show', $product->id) }}" name="detail" class="btn btn-sm btn-oranye btn-move">
                                            <span class="ion-search" aria-hidden="true"></span>
                                        </a>
                                        <button type="submit" form="deleteForm{{ $product->id }}" class="btn btn-sm btn-danger m-2">
